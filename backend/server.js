...
const CLIENT_URL  = process.env.CLIENT_URL  || "https://d2kr25z71q8y4g.cloudfront.net";
const SERVER_URL  = process.env.SERVER_URL  || "http://13.49.77.200:5000";
...
/* ═══════════════════════════════════════════════════
   ...
   http://13.49.77.200:5000/auth/google/callback
═══════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════
   ...
   http://13.49.77.200:5000/auth/github/callback
═══════════════════════════════════════════════════ */
...
app.post("/match", async (req, res) => {
  try {
    ...
    const [internRes, scholarshipRes] = await Promise.all([
      fetch(`${SERVER_URL}/live-internships?field=${encodeURIComponent(profile.field)}`),
      fetch(`${SERVER_URL}/live-scholarships?field=${encodeURIComponent(profile.field)}`)
    ]);
...
app.get("/deadlines", async (req, res) => {
  try {
    ...
    // Fetch internships + scholarships
    const [internRes, scholarshipRes] = await Promise.all([
      fetch(`${SERVER_URL}/live-internships`),
      fetch(`${SERVER_URL}/live-scholarships`)
    ]);
...
═══════════════════════════════════════════════════ */
app.listen(PORT, () => {
  console.log(`Elevatr API running at ${SERVER_URL}`);
