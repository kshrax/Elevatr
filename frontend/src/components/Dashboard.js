...
export default function Dashboard({ profile, user }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`http://13.49.77.200:5000/live-internships?field=${encodeURIComponent(profile?.field || "")}`),
      fetch(`http://13.49.77.200:5000/live-scholarships?field=${encodeURIComponent(profile?.field || "")}`)
    ])
      .then(async ([i, s]) => {
  ...
  useEffect(() => {
    ...
    fetch("http://13.49.77.200:5000/match", {
      method: "POST",
